...
import { Table, Button, Tag, Space, Typography, Card, Input, DatePicker, Modal, Descriptions, message, Spin, Row, Col } from 'antd';
import { SearchOutlined, CheckCircleOutlined, CloseCircleOutlined, EyeOutlined, CarOutlined } from '@ant-design/icons';
...
const ManageOrders = () => {
  ...
  useEffect(() => {
    ...
    if (searchText) {
      result = result.filter(
        order => 
          order.khachHangName?.toLowerCase().includes(searchText.toLowerCase()) ||
        ...
        <Space size="small">
          ...
          {record.trangThai === ORDER_STATUS.CHO_XAC_NHAN && (
            <>
              <Button 
                ...
                onClick={() => handleStatusChange(record.donDatXeId, ORDER_STATUS.DA_XAC_NHAN)}
              ...
              <Button 
                ...
                onClick={() => handleStatusChange(record.donDatXeId, ORDER_STATUS.HUY)}
                loading={confirmLoading}
              >
                Hủy
              </Button>
          ...
          {record.trangThai === ORDER_STATUS.DA_XAC_NHAN && (
            <Button 
              ...
              size="small"
              icon={<CarOutlined />}
              onClick={() => handleStatusChange(record.donDatXeId, ORDER_STATUS.DANG_THUE)}
              loading={confirmLoading}
            >
              Giao xe
            </Button>
          ...
          {record.trangThai === ORDER_STATUS.DANG_THUE && (
            <Button 
              type="default" 
              size="small"
              onClick={() => handleStatusChange(record.donDatXeId, ORDER_STATUS.HOAN_THANH)}
              loading={confirmLoading}
            >
              Hoàn thành
            </Button>
            ...
            <Row gutter={[16, 16]}>
              <Col span={24}>
                <Card title="Thông tin cơ bản" bordered={false}>
                  <Descriptions column={2}>
                    <Descriptions.Item label="Khách hàng">{order.khachHangName}</Descriptions.Item>
                    <Descriptions.Item label="Nhân viên">{order.nguoiDungName || 'N/A'}</Descriptions.Item>
                    <Descriptions.Item label="Trạng thái">{getStatusTag(order.trangThai)}</Descriptions.Item>
                    <Descriptions.Item label="Tổng tiền">{order.tongTien?.toLocaleString('vi-VN')} VNĐ</Descriptions.Item>
                    <Descriptions.Item label="Ngày bắt đầu">{formatDate(order.ngayBatDau)}</Descriptions.Item>
                    <Descriptions.Item label="Ngày kết thúc">{formatDate(order.ngayKetThuc)}</Descriptions.Item>
                    <Descriptions.Item label="Địa điểm nhận xe">{order.diaDiemNhanXe}</Descriptions.Item>
                    <Descriptions.Item label="Địa điểm trả xe">{order.diaDiemTraXe || 'Như địa điểm nhận'}</Descriptions.Item>
                  </Descriptions>
                </Card>
              </Col>
              ...
              <Col span={24}>
                <Card title="Thông tin xe thuê" bordered={false}>
                  {order.mauXeName ? (
                    <Descriptions column={2}>
                      <Descriptions.Item label="Tên xe">{order.mauXeName}</Descriptions.Item>
                      <Descriptions.Item label="Hãng xe">{order.hangXeName}</Descriptions.Item>
                      <Descriptions.Item label="Biển số">{order.bienSo || 'N/A'}</Descriptions.Item>
                      <Descriptions.Item label="Số ngày thuê">{order.soNgayThue || 'N/A'}</Descriptions.Item>
                    </Descriptions>
                  ) : (
  ...
  return (
    <Card>
      ...
      <div className="mb-4">
        <Space size="large">
          <Tag color="blue">{orders.filter(o => o.trangThai === ORDER_STATUS.CHO_XAC_NHAN).length} đơn chờ xác nhận</Tag>
          <Tag color="orange">{orders.filter(o => o.trangThai === ORDER_STATUS.DA_XAC_NHAN).length} đơn đã xác nhận</Tag>
          <Tag color="purple">{orders.filter(o => o.trangThai === ORDER_STATUS.DANG_THUE).length} đơn đang cho thuê</Tag>
          <Tag color="green">{orders.filter(o => o.trangThai === ORDER_STATUS.HOAN_THANH).length} đơn hoàn thành</Tag>
          <Tag color="red">{orders.filter(o => o.trangThai === ORDER_STATUS.HUY).length} đơn đã hủy</Tag>
        </Space>
      </div>
